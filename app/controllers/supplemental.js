			...
			db.Vote.findAll({
				...
				db.Proposal.find({
					...
					db.Item.findAll({
						...
						}).then(function(itemsProp) {
							if (proposal.PartialFunded != undefined && proposal.PartialFunded != null) {
								db.Partial.find({
									where: {
										ProposalId: proposal.id
									}
...
router.get('/ASDFASDF/view/:supplemental', function(req, res) {
	//find the supplemental
	db.Supplemental.find({
...
function getSupplementalProposals(next) {
	db.sequelize.query('SELECT p.Year, ' +
	                          'p.Number, ' +
	                          's.id, ' +
	                          's.Author, ' +
	                          's.Title, ' +
	                   ...
	                   'FROM STF.Supplementals s ' +
	                   'JOIN STF.Proposals p ON s.ProposalId = p.id ' +
	                   'LEFT JOIN STF.Votes v ON s.id = v.SupplementalId ' +
	                   'WHERE s.Submitted = 1 AND s.Status = 0 ' +
	                   'GROUP BY s.id ORDER BY voteCount')
